...
- **结构变量对即时遵从影响有限**：[McMillan, 2026](https://arxiv.org/abs/2605.10039) 在 Claude Code（1,650 会话）做因子实验，未检测到文件大小、指令位置、文件架构对**单条指令遵从**的效应（size/conflict 的 null 有贝叶斯因子支持）。这与上述成本证据不冲突——后者测的是整体推理开销与成功率，而非单条指令的即时遵从。结论：精简仍有成本价值，但不必过度优化指令的具体位置。
