...
- **手写精简有益**：[R2] 测得手写 AGENTS.md 降低运行时间与 token 消耗，且不损质量。
- **冗余内容有害**：[R3] 发现不必要指令推高推理成本，自动生成更是一致降低成功率。
- **结构变量对即时遵从影响有限**：[R4] 未检测到文件大小/位置/架构对单条指令遵从的效应。这与上述成本证据不冲突——测的维度不同。结论：精简有成本价值，但不必过度优化指令位置。
