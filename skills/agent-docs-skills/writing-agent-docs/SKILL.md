...
遇到问题先查这张表，再按编号定位规则（括注为失败模式术语，定义见 [L7]）：

| 症状 / 场景 | 看哪条 |
|---|---|
| 想为“以防万一”的假设场景写规则 | A.1 |
| 规则能被 linter / hook / schema 强制（指令遵从缺陷）| A.2 |
| 内容代理能从代码或预训练读到（上下文衰减）| A.3 |
| 规则越列越长，想给个总则（提示词脆弱性）| A.4 |
| 表述怕过时（时间混淆）| M.1 / B.5 |
| 否定指令代理反而做了（否定 prime）| B.2 |
| 占位符被代理字面复制（提示词脆弱性）| B.3 |
| 关键规则代理没看到（位置偏置）| C.3 |
| 代理说完成其实没验证（腐败成功 / 过度自信）| C.5 |
| 文档不知该删什么 | 维护·腐败信号表 |
...
- **每 token 须自证价值**：找到“最大化期望结果的最小高信号 token 集”
- **文档即承诺**：每写一条规则就是承诺维护它——写之前问“值得长期维护吗”，代理已能遵循的条目应移除（增量迭代见 A.1）

> 这些原则是 **context engineering**（[R1]）的具体应用。其紧迫性见 [R8]——IBM Research 指出 agent 文档无序泛滥已成系统性危机，本技能的核心原则正是其解药。
>
> 每条规则都在对抗一类 **LLM 失败模式**——各节以 `**根因：**` 标注所对抗的术语，定义见 [L7]。
...
**根因：** 时间混淆（[L7]）— 模型无内部时间感；事实陈述会过时，判断标准不会

描述“什么是对的”（判断标准），而非“世界是什么样的”（当前状态）——判断标准不过时，当前状态必然过时。

本条规则跨三域适用——选材时问内容能否升维，雕琢时问表述能否升维，架构时问组织能否升维。

**判断方法**：删掉所有数字和专有名词后，核心指导是否仍然成立？成立则升维为原则陈述。

```markdown
# 事实陈述（需维护）
SKILL.md 开放标准已被数十款工具原生支持。
# 原则陈述（不维护）
SKILL.md 是开放标准——跨工具兼容是其设计目标，优先用它分发技能。
```

需要引用事实时，用引用编号承载，正文只说原则。**例外**：操作性信息（怎么安装）、证据性信息（安全警示精确数字）、路由性信息（用哪个文件）必须留在事实层。
...
**根因：** 上下文衰减（[L7]）— 推测规则膨胀上下文，且未经真实失败验证
...
**根因：** 指令遵从缺陷（[L7]）— 复杂多约束最佳模型完美遵从 <30%；机制要求 > 指令请求
...
**根因：** 上下文衰减（[L7]）— 重复模型已知内容浪费 token 并与源码漂移
...
**根因：** 提示词脆弱性（[L7]）— 穷举细节易漂移；整体约束抗扰动、更易维护
...
**选材实操速查**——面对一段待写内容，按序检查：

| 检查 | 不通过则 |
|------|---------|
| 代理是否反复在同一处失败？（A.1）| 不写——推测性规则 |
| 能否被 linter/CI/hook 强制？（A.2）| 不写——指向工具配置 |
| 代理能否从代码/预训练自行获取？（A.3）| 不写——浪费 token |
| 能否给整体约束让代理推理？（A.4）| 不穷举——给约束 |
| 能否升维为原则陈述？（M.1）| 不写事实——升维或用引用编号 |

### B. 雕琢域 — 决定怎么写

#### B.1 简洁优先

**根因：** 上下文衰减 + 显式行动指令（[L7]）— 命令可自验证，描述需代理推断
...
**根因：** 行为偏置·否定 prime（Pink Elephant，[L7]）— 提及禁用词本身会 prime 模型产生它（[R2]）
...
**根因：** 提示词脆弱性 + 领域术语（[L7]）— 真实值锚定语义边界；占位符会被字面复制（[R4]）

- **正反示例优先于纯文字解释**——一个正反示例胜过三段文字描述
- 示例要完整可运行、来自真实场景、注释解释“为什么”
- **避免“WRONG:”反例单独出现**——它会将要避免的模式注入上下文；有正确示例时，无需反例
- **示例用真实值，禁用占位符**——禁用 `"string"`、`"YOUR_VALUE"`、`"example-slug"`，使用项目真实数据
...
**根因：** 显式行动指令（[L7]）— 命令带 flag 可自验证、可 grep
...
**根因：** 时间混淆 + 领域术语（[L7]）— 过时指令比没有更糟；术语一致锚定语义边界
...
| 检查 | 不通过则 |
|------|---------|
| 能用一句话说清？（B.1）| 精简——命令优先于描述 |
| 含否定指令？配了肯定替代？（B.2）| 补替代或改肯定 |
| 有正反示例？用了真实值？（B.3）| 补示例、去占位符 |
| 路径用正斜杠？命令带 flag？（B.4）| 修正格式 |
| 含版本号/日期？术语一致？（B.5）| 去时间标记、统一术语 |
| 句式可用升维为原则陈述？（M.1）| 去精确值——升维或用引用编号 |

### C. 架构域 — 决定怎么组织

#### C.1 验证反馈循环

**根因：** 静默失败·复合衰减（[L7]）— 多步任务错误累积；验证回路打断复合
...
**根因：** 上下文衰减·Lost-in-the-Middle（[L7]）— 重内容下沉，入口精简以避开中部衰减

重内容下沉到子文件 / 子目录，入口保持精简，按需加载。具体机制（文件系统层级 / 三级加载 / 引用深度）见领域 skill。

#### C.3 结构与信息架构

**根因：** 位置偏置（[L7]）— 长上下文首尾注意力强、中部最弱（U 型曲线）

- Markdown 分节
- **原子任务**：不合并可独立完成的步骤
- **bookend**：关键规则放文件顶部和任务区前
...
**根因：** 领域术语（[L7]）— 代理解析结构化数据比无序列举更可靠（[R4]）
...
**根因：** 静默失败·腐败成功 / 过度自信（[L7]）— 代理未经验证即报告“完成”
...
| 检查 | 不通过则 |
|------|---------|
| 路径/命令真实存在？有验证步骤？（C.1）| 补验证 |
| 入口精简？重内容下沉？（C.2）| 拆分到子文件 |
| 按目标任务组织？步骤有真顺序？（C.3）| 重组结构 |
| 用了表格/结构化而非长 bullet list？（C.4）| 重组为表格 |
| 工作流有完成标准 + 阻塞升级？（C.5）| 补配对 |
| 内容层面含可升维的事实表述？（M.1）| 下沉到参考文件或升维为原则 |
...
> 工具链优先、Always/Ask/Never 边界等**领域专属规则**见上述子 skill。
>
> **互补参考**：交付层标准（文档站点面向 agent 的检查项）见 [R10]/[L5]；Addy Osmani 的规范写作 5 原则见 [R9]；本技能的内容层原则（简洁 B.1、结构化 C.4）同时优化人读与 RAG 检索，无需为 RAG 另设规则。
...
| [L7] | [llm-failure-modes.md](./references/llm-failure-modes.md) | LLM 失败模式术语表（文档相关子集）——各规则 `**根因：**` tagline 所用术语的唯一定义处，含 5 类失败模式 + 2 条优化支柱 |
