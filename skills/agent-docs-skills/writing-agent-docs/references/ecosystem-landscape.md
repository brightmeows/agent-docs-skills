...
当前，公开追踪的 AI agent 工具生态由三大类构成：

| 类型 | 规模 | 职责 |
|------|------|------|
| **Agent Skills** | 数万级，累计安装量领先 | 指令与工作流（SKILL.md），教会 agent **做什么** |
| **MCP Servers** | 数千级，快速增长 | 外部工具与数据连接，给 agent **用什么** |
| **Marketplaces** | 数千级 | 分发、发现与安全扫描 |

> 数据口径说明：[R1] 追踪经过注册/索引的活跃技能；若计入全 GitHub 自动抓取，公开 SKILL.md 文件总量达百万级（[R3]）。两口径互补——前者反映活跃技能，后者反映 GitHub 原始总量。精确数字见参考表。
> 质量参差——公开技能平均评分偏低，仅顶部四分之一有实质提升（[R2]）。
