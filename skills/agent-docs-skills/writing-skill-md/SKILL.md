...
**编写技能是为代理编写可发现、可执行、可遵从的参考指南。** 技能好坏取决于三点：代理能否**找到**（CSO）、**理解**（结构）、**遵从**（清晰）。

**验证思维：** 先想清楚代理在没有这个技能时会怎么失败，再写最简的指导（借鉴 TDD）。完整 TDD subagent 压力测试作为人类作者参考见[L5]。

技能可放项目级（仓库内 `skills/` 或 `.claude/skills/`）或个人级（`~/.claude/skills/`、`~/.agents/skills/`），见[作用域选择](#作用域选择)。Anthropic 官方最佳实践见[L1]，本文档提供补充模式。
...
**分类维度是加载时机，不是内容来源**——`references/` 收纳所有按需加载文档，不区分自撰与转载；参考文件多时按**主题**分子目录（[R2]），不按来源分。纯参考型技能（SKILL.md 为索引）可让主题目录做顶层（如 `memo/`）。不确定时参考 [R3]。

官方未覆盖的边界（Gherkin 规格、示例数据）可建自定义目录（`test/`、`samples/`），在 SKILL.md 说明用途与加载时机。

**归置优先级**：先就地归入当前文件相关章节；同文件无合适归处时按下表下沉：

| 类型 | 去向 |
|---|---|
| 重量级参考（>100 行）| `references/` |
| 可复用工具/脚本 | `scripts/` |
| 静态资源 | `assets/` |

原则、概念、短代码模式（≤50 行）保持内联。

**引用保持一层深度**——SKILL.md 到目标文件不超过 1 跳（`SKILL.md → references/foo.md`，避免 `→ references/sub/foo.md → bar.md`）。文件系统路径深度不影响引用链长度。嵌套引用会导致代理用 `head` 预览，信息不完整。
...
**主体大小**：SKILL.md body 保持 <500 行 / **指令 < 5000 tokens**（agentskills.io）；接近上限时拆分到参考文件（超 100 行的参考文件加目录）。

**description 长度**：字段上限 1024 字符，建议 ≤**200 字符**（短描述更易被代理扫描）。**权衡**：过短会牺牲触发精度——先保证触发可靠，再压缩到 200 字符内。（Hermes Agent 另有 ≤60 字符的工具特有标准。）
...
|---|---|
| 概述 | 这是什么 + 核心原则一两句 |
| 何时使用 | 症状、用例、何时不使用；决策不明显时配流程图 |
| 核心模式 | 改进前后代码对比（技术/模式型）|
| 快速参考 | 常用操作表格（优先表格，writing-agent-docs C.4）|
| 实现 | 内联代码或文件链接 |
| 常见错误 | 出错点 + 修复 |
| 实际效果（可选）| 具体结果 |
| 速查入口（可选）| 概述后放“症状→章节”表，一眼定位 |

**速查入口**置于概述后，命中长上下文首部高注意力区与症状关键词检索（writing-agent-docs C.3 bookend、C.4 结构化），适用于规则/章节多的技能；短技能无需。
...
技能直接注入代理上下文，恶意或脆弱的技能可致数据窃取、权限提升。研究已证实主流扫描器均可被绕过（CSA、Trail of Bits），且有虚假技能绕过所有检测、触及 26,000 个 agent 的真实攻击（AIR）；OWASP 已发布 **Agentic Skills Top 10（AST10）** 行业标准。完整证据链与应对见 [security.md](./references/security.md)。
