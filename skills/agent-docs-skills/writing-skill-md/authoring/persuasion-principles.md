...
**研究基础：** [R1] 在 N=28,000 个 AI 对话中测试了 7 项说服原则。说服技术使遵从率提升至两倍以上（33% → 72%，p < .001）。
...
```markdown
好：完成任务后，IMMEDIATELY 请求代码审查，之后再继续。
坏：你可以在方便时审查代码。
```
...
```markdown
好：清单无 TodoWrite 跟踪 = 步骤被跳过。每一次。
坏：有些人觉得 TodoWrite 对清单有用。
```
