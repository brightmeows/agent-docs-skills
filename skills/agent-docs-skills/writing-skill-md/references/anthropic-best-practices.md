...
**类比**：把代理想象成探索路径的机器人——两边是悬崖的窄桥（如数据库迁移）给低自由度的精确护栏；没有危险的广阔田野（如代码审查）给高自由度的大致方向，相信它找到最佳路线。
...
预制脚本比让代理现场生成更可靠、省 token、确保一致性。在指令中明确代理应**执行脚本**（最常见）还是**作为参考阅读**（用于复杂逻辑）：
...
当输入可渲染为图像时，让代理分析它们（视觉能力有助于理解布局和结构）：
...
- **确定性操作首选脚本**：写 `validate_form.py` 而非让代理生成验证代码
- **测试文件访问**：通过真实请求验证代理能导航你的目录结构
