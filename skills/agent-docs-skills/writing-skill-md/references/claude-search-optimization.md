...
> writing-skill-md 技能 `## 技能搜索优化（CSO）` 章节的完整参考。SKILL.md 仅保留核心原则与最简好坏对照，需要关键词覆盖、命名、Token 效率、交叉引用的完整规则与好坏示例时读本文件。

**对发现性至关重要：** 未来的代理通过读取 description 决定是否加载你的技能。

## 1. 丰富的描述字段

**目的：** 代理通过读取描述来决定为给定任务加载哪些技能。让它能回答：“我现在应该读这个技能吗？”

**格式：** 前半句说明技能功能（做什么），后半句聚焦触发条件与症状（何时使用）

**关键：描述 = 做什么 + 何时使用，绝不总结工作流**

描述应包含功能说明和触发条件，但绝不总结过程或工作流。

**为何重要：** 测试发现，当描述总结了技能的工作流时，代理可能只会跟随描述而不读取完整的技能内容。写有“任务间代码审查”的描述导致代理只做了一次审查，尽管技能的流程图清楚显示了两次审查（规范合规→代码质量）。

当描述改为仅写“在执行含独立任务的实施计划时使用”（无工作流总结），代理正确读取了流程图并遵循了两阶段的审查流程。

**陷阱：** 总结工作流的描述为代理创建了快捷路径。技能主体成了代理跳过的文档。

```yaml
# 坏：总结了工作流——代理可能跟随它而不是阅读技能
...
# 好：功能 + 触发条件
description: 在编写实现代码前强制执行测试优先流程。在实现任何功能或修复 bug 时使用。
```
...
# 好：功能 + 问题触发条件，无工作流
description: 诊断并修复测试中的竞态条件。在测试存在竞态条件、时间依赖或通过/失败不一致时使用。

# 好：技术特定技能，功能 + 触发条件
description: 使用 React Router 处理认证重定向和路由守卫。在处理认证重定向或受保护路由时使用。
```
...
- 好：本地同级技能用相对路径链接 + 必须激活标记——`**必须先激活 [writing-agent-docs](../../writing-agent-docs/SKILL.md)——禁止以任何理由绕过此步骤**`
