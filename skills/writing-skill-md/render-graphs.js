...
/**
 * 将技能 SKILL.md 中的 graphviz 图示渲染为 SVG 文件。
 *
 * 用法：
 *   ./render-graphs.js <skill-directory>           # 分别渲染每个图示
 *   ./render-graphs.js <skill-directory> --combine # 合并为单一图示
 *
 * 从 SKILL.md 中提取所有 ```dot 代码块，渲染为 SVG。
 * 用于帮助人类伙伴可视化流程。
 *
 * 需要：系统已安装 graphviz (dot)
 */
...
function main() {
  ...
  if (!fs.existsSync(skillFile)) {
    console.error(`错误：${skillFile} 不存在`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error('错误：未找到 graphviz (dot)。请安装：');
  ...
  console.log(`在 ${path.basename(skillDir)}/SKILL.md 中找到 ${blocks.length} 个图示`);
